...
class checkEmailAuthConflict {
  ...
  changeUser() {
    ...
    fetch(`/checkout/changeToAnonymousUser/${_this.orderForm.orderFormId}`)
      .then(() => {
        _this.removeModal()
        vtexid.start()
      })
      .catch(() => {
        // if fetch errors, fallback to redirect
        window.location.replace(`/checkout/changeToAnonymousUser/${_this.orderForm.orderFormId}`)
      })
  }
  ...
  validate() {
    const _this = this
    try {
      if (_this.orderForm && _this.orderForm.clientProfileData && _this.orderForm.clientProfileData.email) {
